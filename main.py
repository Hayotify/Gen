...
class MusicBot(BaseBot):
    def __init__(self):
        ...
        self.song_queue = []        # Music queue [{user, song, duration, link}]
        self.stream_process = None  # Active FFmpeg subprocess
        self.stream_start_time = 0  # When current stream started
        self.dj_playlist = []       # Auto-DJ playlist loaded from dj.txt
        self.dj_index = 0           # Current position in dj_playlist
        self.dj_enabled = True      # Auto-DJ on/off toggle
        self.load_dj_playlist()     # Load saved playlist on startup
        self.np_likes = 0           # Likes for current song
        self.np_dislikes = 0        # Dislikes for current song
        self.np_liked_users = set() # Users who liked (one vote per song)
        self.np_disliked_users = set()
        self.welcome_messages = [
            "Welcome to {room}, {user}! Enjoy your stay!",
            "Hello {user}! So glad you joined {room} today.",
            "Welcome {user}! Feel free to vibe and loop some emotes in {room}.",
            "Hey {user}! Welcome to {room}!",
            "Welcome, {user}! You're now in {room}. Type -help to see what I can do.",
            "Glad to see you here in {room}, {user}!",
            "Welcome {user}! Have a great time in {room}!",
            "Hey there {user}! Welcome to {room}.",
            "A warm welcome to you, {user}! This is {room}.",
            "Welcome {user}! Don't forget to check out our -emotelist.",
            "Yo {user}! Ready to dance in {room}? 💃",
            "Welcome {user}! The music is just getting started in {room}!",
            "Welcome {user}! Hope you make some new friends here in {room}.",
            "Hey {user}! Looking good today! ✨ Welcome to {room}!",
            "Welcome to the squad at {room}, {user}!",
            "High five {user}! Welcome to {room}! ✋",
            "Welcome {user}! Make yourself at home in {room}.",
            "Hey {user}! We've been waiting for you in {room}! 🌟",
            "Welcome {user}! Stay as long as you like in {room}.",
            "Welcome {user}! Good vibes only in {room}! 🌈",
            "Welcome {user}! The party at {room} just got 10x better! 🎉",
            "Oh look! {user} is here to save the day at {room}! 🦸‍♂️",
            "Welcome {user}! Keep it active and fun in {room}! 🔥",
            "Hey {user}! VIP vibes only in {room}, glad you're here! ✨",
            "Welcome {user}! Let's make some memories in {room} today! 📸",
            "Yo {user}! Don't just stand there, let's dance in {room}! 💃🕺",
            "Welcome {user}! You're just in time for the best part of {room}! 🎊",
            "Welcome {user}! Grab a drink and let's vibe in {room}! 🍹",
            "Hey {user}! Stay active and enjoy the music in {room}! 🎶",
            "Welcome {user}! The legend has arrived at {room}! 🏆"
        ]
        ...
        try:
            with open(self.config_path, "r") as f:
                ...
                if "OWNER" in self.config and "OWNERS" not in self.config:
                    ...
                    self.owners = self.config.get("OWNERS", ["harmanpreet_19"])
    ...
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        ...
        try:
            ...
            # Start music: auto-DJ queue and periodic now-playing announcer
            asyncio.create_task(self.auto_dj_loop())
            asyncio.create_task(self.song_announcer_loop())
    ...
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        ...
        try:
            ...
            # Send designation-specific welcome
            if designation == "owner":
                await self.highrise.chat(
                    f"Welcome back, Room Owner @{user.username}! 👑 The boss has arrived in {self.room_name}!"
                )
            elif designation == "moderator":
                await self.highrise.chat(
                    f"Welcome back, Room Moderator @{user.username}! 🛡️ Thanks for keeping {self.room_name} safe!"
                )
            else:
                welcome_msg = random.choice(self.welcome_messages).format(user=user.username, room=self.room_name)
    ...
    async def run_advertisement_loop(self):
        """Send a promotional message every 7 minutes."""
        await asyncio.sleep(60)
        while True:
            try:
                await self.highrise.chat("🎶 Feel free to request your favorite song anytime — it's FREE! Just type -play <song name> and let the music play! 🎼")
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                await self.highrise.chat(f"✅ Look applied (Safe Mode)! Some body/face items were incompatible.")
                            except:
            ...
            if message.lower() == "-help":
                ...
                help_text += "🎵 **MUSIC**\n"
                help_text += "-play <song>: Search & queue a song\n"
                help_text += "-np: See what's currently playing\n"
                help_text += "-next: Preview the upcoming song\n"
                help_text += "-q: View the full song queue\n"
                help_text += "-skip: Skip current song (mod/owner)\n"
                help_text += "-clear: Clear queue (mod/owner)\n"
                help_text += "-like ❤️: Like the current song\n"
                help_text += "-dislike 👎: Dislike the current song\n\n"

                help_text += "🎛️ **AUTO-DJ**\n"
                help_text += "-setdj <playlist url>: Load a YouTube playlist (owner)\n"
                help_text += "-djon / -djoff: Toggle Auto-DJ on/off\n"
                help_text += "-djlist: Show playlist & upcoming songs\n"
            ...
            # Teleportation and Location Management
                ...
                if not bar_id:
                    await self.highrise.chat(f"Invalid amount. Allowed: 1, 5, 10, 50, 100, 500, 1000, 5000, 10000")
                    return
            ...
            # ----- Music Commands -----
                ...
                if not song_query:
                    await self.highrise.chat(f"@{user.username} Please specify a song! Example: -play Despacito")
                    return
                if len(self.song_queue) >= 11:
                    await self.highrise.chat(f"⚠️ @{user.username} The queue is full (max 10)! Wait for some songs to finish.")
                    return
                await self.highrise.chat(f"🔍 @{user.username} Searching for '{song_query}'...")
                yt_title, yt_link, yt_duration = await self.search_youtube_song(song_query)
                if not yt_title:
                    await self.highrise.chat(f"❌ @{user.username} Sorry, couldn't find that song.")
                    return
                queue_pos = len(self.song_queue)
                pos_text = "Now Playing" if queue_pos == 0 else f"Position: {queue_pos}"
                self.song_queue.append({"user": user.username, "song": yt_title, "duration": yt_duration, "link": yt_link})
                await self.highrise.chat(f"🎵 Added to Queue!\nSong: {yt_title}\n{pos_text}")

            elif message.lower() in ["-q", "-queue"]:
                if len(self.song_queue) <= 1:
                    await self.highrise.chat("The song queue is currently empty!")
                    return
                q_msg = "🎵 Song Queue 🎵\n"
                for i, s in enumerate(self.song_queue[1:11]):
                    title = s.get("song", "Unknown")
                    if len(title) > 25:
                        title = title[:22] + "..."
                    q_msg += f"{i + 1}. {title} (@{s.get('user', '?')})\n"
                if len(self.song_queue) > 11:
                    q_msg += f"...and {len(self.song_queue) - 11} more!"
                await self.highrise.chat(q_msg.strip())

            elif message.lower() == "-np":
                if not self.song_queue:
                    await self.highrise.chat("🔇 Nothing is playing right now!\nType -play <song name> to request one.")
                    return
                await self.highrise.chat(await self.format_np_message(self.song_queue[0]))

            elif message.lower() == "-next":
                if len(self.song_queue) <= 1:
                    await self.highrise.chat("No upcoming songs in the queue!")
                    return
                s = self.song_queue[1]
                mins, secs = divmod(s.get("duration", 0), 60)
                await self.highrise.chat(f"⏭️ Next Up\nSong: {s.get('song', '?')}\nDuration: {mins}:{secs:02d}\nRequested by: @{s.get('user', '?')}")

            elif message.lower() == "-skip":
                if not await self.is_mod_or_owner(user.id, user.username):
                    await self.highrise.chat(f"🚫 @{user.username} Only owners and moderators can skip songs!")
                    return
                if not self.song_queue:
                    await self.highrise.chat("No song to skip!")
                    return
                skipped = self.song_queue.pop(0)
                await self.stop_stream()
                self.reset_np_votes()
                await self.highrise.chat(f"⏭️ @{user.username} skipped: {skipped['song']}")
                if self.song_queue:
                    await self.highrise.chat(f"▶️ Up next: {self.song_queue[0]['song']}")
                else:
                    await self.highrise.chat("The queue is now empty.")

            elif message.lower() == "-clear":
                if not await self.is_mod_or_owner(user.id, user.username):
                    await self.highrise.chat(f"🚫 @{user.username} Only owners and moderators can clear the queue!")
                    return
                if len(self.song_queue) > 1:
                    self.song_queue = self.song_queue[:1]
                    await self.highrise.chat(f"🧹 @{user.username} cleared the song queue!")
                else:
                    await self.highrise.chat("The queue is already empty.")

            elif message.lower() in ["-like", "❤️"]:
                if not self.song_queue:
                    await self.highrise.chat("No song is playing right now!")
                    return
                uname = user.username.lower()
                if uname in self.np_liked_users:
                    await self.highrise.chat(f"@{user.username} You already liked this song! ❤️")
                    return
                self.np_disliked_users.discard(uname)
                if uname in self.np_disliked_users:
                    self.np_dislikes = max(0, self.np_dislikes - 1)
                self.np_liked_users.add(uname)
                self.np_likes += 1
                await self.highrise.chat(f"❤️ @{user.username} liked the song! ({self.np_likes} likes)")

            elif message.lower() in ["-dislike", "👎"]:
                if not self.song_queue:
                    await self.highrise.chat("No song is playing right now!")
                    return
                uname = user.username.lower()
                if uname in self.np_disliked_users:
                    await self.highrise.chat(f"@{user.username} You already disliked this song! 👎")
                    return
                if uname in self.np_liked_users:
                    self.np_liked_users.discard(uname)
                    self.np_likes = max(0, self.np_likes - 1)
                self.np_disliked_users.add(uname)
                self.np_dislikes += 1
                await self.highrise.chat(f"👎 @{user.username} disliked the song. ({self.np_dislikes} dislikes)")

            elif message.lower().startswith("-setdj "):
                if not self.is_owner(user.username):
                    await self.highrise.chat(f"🚫 @{user.username} Only owners can set the Auto-DJ playlist.")
                    return
                playlist_url = message[7:].strip()
                if not playlist_url:
                    await self.highrise.chat("Usage: -setdj <YouTube playlist URL>")
                    return
                await self.highrise.chat(f"🎛️ Fetching playlist... this may take a moment!")
                try:
                    count = await self.fetch_and_save_playlist(playlist_url)
                    if count == 0:
                        await self.highrise.chat("❌ Could not find any songs in that playlist. Check the URL and try again.")
                    else:
                        await self.highrise.chat(f"✅ Auto-DJ playlist saved! {count} songs loaded from the playlist.\nAuto-DJ will kick in whenever the request queue is empty.")
                except Exception as e:
                    await self.highrise.chat(f"❌ Error fetching playlist: {e}")

            elif message.lower() == "-djon":
                if not await self.is_mod_or_owner(user.id, user.username):
                    await self.highrise.chat(f"🚫 @{user.username} Only owners and moderators can control Auto-DJ.")
                    return
                self.dj_enabled = True
                status = f"🎛️ Auto-DJ is now ON. {len(self.dj_playlist)} songs in playlist." if self.dj_playlist else "🎛️ Auto-DJ is ON but no playlist is set. Use -setdj <url> to load one."
                await self.highrise.chat(status)

            elif message.lower() == "-djoff":
                if not await self.is_mod_or_owner(user.id, user.username):
                    await self.highrise.chat(f"🚫 @{user.username} Only owners and moderators can control Auto-DJ.")
                    return
                self.dj_enabled = False
                await self.highrise.chat("🎛️ Auto-DJ is now OFF. Only manual -play requests will be queued.")

            elif message.lower() == "-djlist":
                if not self.dj_playlist:
                    await self.highrise.chat("🎛️ No Auto-DJ playlist loaded. Use -setdj <YouTube playlist URL> to set one.")
                    return
                status = "ON ✅" if self.dj_enabled else "OFF ❌"
                total = len(self.dj_playlist)
                current_pos = self.dj_index % total
                preview = []
                for i in range(min(5, total)):
                    idx = (current_pos + i) % total
                    title = self.dj_playlist[idx]["song"]
                    if len(title) > 28:
                        title = title[:25] + "..."
                    preview.append(f"{i + 1}. {title}")
                await self.highrise.chat(
                    f"🎛️ Auto-DJ [{status}]\n"
                    f"Playlist: {total} songs\n"
                    f"Up next:\n" + "\n".join(preview)
                )
    ...
    def reset_np_votes(self):
        """Reset like/dislike counts when a new song starts."""
        self.np_likes = 0
        self.np_dislikes = 0
        self.np_liked_users.clear()
        self.np_disliked_users.clear()

    async def format_np_message(self, song: dict) -> str:
        """Build the colored Now Playing card matching the room's display style."""
        title     = song.get("song", "Unknown")
        requester = song.get("user", "?")
        duration  = song.get("duration", 0)
        is_dj     = "Auto-DJ" in requester
        ...
        d_m, d_s = divmod(duration, 60)
        elapsed_str = f"{e_m:02d}:{e_s:02d}"
        total_str   = f"{d_m:02d}:{d_s:02d}"
        bar_width   = 12
        dot_pos     = min(int(pct / 100 * bar_width), bar_width - 1)
        bar         = "─" * dot_pos + "◉" + "─" * (bar_width - 1 - dot_pos)

        # --- Listener count ---
        try:
            room_users = await self.highrise.get_room_users()
            listeners = len(room_users.content) if not isinstance(room_users, Error) else 0
        except Exception:
            listeners = 0
        ...
        if is_dj:
            ...
            req_line = f"👤 𝙍𝙚𝙦𝙪𝙚𝙨𝙩: @{requester}"

        return (
            "✨ ━━ 🎧 𝐍𝐎𝐖 𝐏𝐋𝐀𝐘𝐈𝐍𝐆 ━━ ✨\n\n"
            f"<🎶 𝙎𝙤𝙣𝙜: {title}\n"
            f"{req_line}\n\n"
            f"{elapsed_str} {bar}{total_str} ({pct}%)\n"
            "⟲       ◁         ‖         ▷       ⟳\n\n"
            f"❤️ {self.np_likes}   |   👎 {self.np_dislikes}   |   🎧 {listeners} Listens"
        )

    def load_dj_playlist(self):
        """Load the Auto-DJ playlist from dj.txt into memory."""
        try:
            if os.path.exists("dj.txt"):
                with open("dj.txt", "r", encoding="utf-8") as f:
                    entries = []
                    for line in f:
                        line = line.strip()
                        if not line:
                        ...
                        if len(parts) == 3:
                            title, url, dur = parts
                            entries.append({"song": title, "url": url, "duration": int(dur), "link": url})
                self.dj_playlist = entries
                if entries:
                    print(f"[Auto-DJ] Loaded {len(entries)} songs from dj.txt")
        except Exception as e:
            print(f"[Auto-DJ] Failed to load dj.txt: {e}")

    def extract_playlist_url(self, raw: str) -> str:
        """Given any YouTube URL or bare playlist ID, return a clean playlist URL."""
        raw = raw.strip()
        # If it's already just a playlist ID (starts with PL, RD, UU, OL, FL, etc.)
        if raw.startswith(("PL", "RD", "UU", "OL", "FL")) and " " not in raw and "/" not in raw:
            return f"https://www.youtube.com/playlist?list={raw}"
        # Parse as URL and extract the list= query param
        try:
    ...
    async def fetch_and_save_playlist(self, playlist_url: str) -> int:
        ...
        for e in entries:
            ...
            # Sanitise title so it can't break the pipe-delimited format
            title = title.replace("|||", " ")
            lines.append(f"{title}|||{url}|||{duration}")

        with open("dj.txt", "w", encoding="utf-8") as f:
            f.write("\n".join(lines))

        self.load_dj_playlist()
        self.dj_index = 0
        return len(lines)

    def next_dj_song(self) -> dict | None:
        """Return the next Auto-DJ song (cycling), or None if playlist is empty."""
        if not self.dj_playlist:
            return None
        song = self.dj_playlist[self.dj_index % len(self.dj_playlist)]
        self.dj_index = (self.dj_index + 1) % len(self.dj_playlist)
        return {
            "user": "Auto-DJ 🎛️",
            "song": song["song"],
    ...
    async def search_youtube_song(self, query: str):
        """Search YouTube for a song and return (title, url, duration)."""
        ...
        try:
            ...
            if info:
                ...
                url = info.get('webpage_url') or (query if is_url else f"https://www.youtube.com/watch?v={vid_id}")
                return title, url, int(duration) if duration else 210
        except Exception as e:
    ...
    async def get_direct_audio_url(self, youtube_url: str):
        ...
        try:
            ...
            print(f"[Music] Direct URL error: {e}")
        return None

    async def play_song_to_icecast(self, song_info: dict) -> bool:
        """Stream a song from YouTube to the configured Icecast server via FFmpeg."""
        ...
        icecast_url = self.config.get("icecast_url")
        if not icecast_url:
            print("[Music] No icecast_url in config.json")
            return False
    ...
    async def stop_stream(self):
        """Kill the active FFmpeg streaming process."""
        try:
            if self.stream_process and self.stream_process.returncode is None:
                self.stream_process.terminate()
                await asyncio.sleep(0.5)
                if self.stream_process.returncode is None:
    ...
    async def auto_dj_loop(self):
        """Background task: manages the song queue and drives FFmpeg streaming.
        When the request queue is empty and Auto-DJ is enabled, feeds songs
        from the dj.txt playlist automatically."""
        while True:
            await asyncio.sleep(2)
            try:
                # Check if the current stream process ended
                if self.stream_process is not None:
                    if self.stream_process.poll() is not None:
                        self.stream_process = None
                        elapsed = time.time() - self.stream_start_time
                        if elapsed < 10:
                            print(f"[Music] Stream crashed fast ({elapsed:.1f}s), retrying in 5s...")
                            await asyncio.sleep(5)
                        else:
                            print("[Music] Stream finished normally.")
                            if self.song_queue:
                                self.song_queue.pop(0)
                                if self.song_queue:
                                    await self.highrise.chat(f"▶️ Now Playing: {self.song_queue[0]['song']}")

                # If nothing is queued and Auto-DJ is on, pull the next DJ song
                if self.stream_process is None and not self.song_queue:
                    if self.dj_enabled and self.dj_playlist:
                        next_song = self.next_dj_song()
                        if next_song:
                            self.song_queue.append(next_song)
                            print(f"[Auto-DJ] Queuing: {next_song['song']}")

                # Start streaming the next song if nothing is playing
                if self.stream_process is None and self.song_queue:
                    current = self.song_queue[0]
                    self.stream_start_time = time.time()
                    success = await self.play_song_to_icecast(current)
                    if not success:
                        print(f"[Music] Skipping '{current['song']}' — playback failed.")
                        if self.song_queue:
                            self.song_queue.pop(0)
                    else:
                        self.reset_np_votes()
                        await asyncio.sleep(2)
                        await self.highrise.chat(await self.format_np_message(current))
            except Exception as e:
                print(f"[Music] Auto DJ error: {e}")

    async def song_announcer_loop(self):
        """Re-announce the currently playing song every 60 seconds."""
        while True:
            await asyncio.sleep(120)
            try:
                if self.song_queue and self.stream_process and self.stream_process.poll() is None:
                    await self.highrise.chat(await self.format_np_message(self.song_queue[0]))
            except Exception as e:
                print(f"[Music] Announcer error: {e}")
    ...
    while True:
        ...
        try:
            ...
            for line in proc.stdout:
                ...
                # Detect Highrise connection-closed errors
                if "connection" in line_lower and "closed" in line_lower:
                    print(f"[Bot] Connection closed detected — restarting in {restart_delay}s...")
            ...
            if not connection_dropped:
                ...
                print(f"[Bot] Process exited with code {code} — restarting in {restart_delay}s...")
